...
async def handler(request):
    """Handle OAuth callback for Azure"""
    try:
        ...
        if error:
            logger.error(f"OAuth error from Azure: {error}")
            return {
                'statusCode': 400,
                'headers': {
        ...
        token_data = await oauth_manager.exchange_code_for_token('azure', code)
        
        # Get user info from Microsoft Graph
        user_info = await get_azure_user_info(token_data['access_token'])
        ...
        if success:
            ...
            return {
                ...
                <body>
                    <h1>Success!</h1>
                    <p>Microsoft Azure has been connected successfully.</p>
                    <p>You can close this window and return to the dashboard.</p>
                    <script>
                        // Send message to parent window
                        if (window.opener) {
                            window.opener.postMessage({ type: 'oauth_success', provider: 'azure' }, '*');
                        }
            ...
            await supabase_manager.log_activity(
                ...
                activity_type='oauth_completed',
                status='error',
                message='Failed to save Azure integration',
            ...
            return {
                ...
                <body>
                    <h1>Error</h1>
                    <p>Failed to save Azure integration. Please try again.</p>
                    <script>window.close();</script>
                </body>
                </html>
                """
        ...
        logger.error(f"Error in Azure OAuth callback: {str(e)}")
        ...
        return {
            ...
            <body>
                <h1>Error</h1>
                <p>An error occurred while connecting Azure: {str(e)}</p>
                <script>window.close();</script>
            </body>
            </html>
            """
...
async def get_azure_user_info(access_token: str) -> dict:
