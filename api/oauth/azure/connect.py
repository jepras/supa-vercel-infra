...
async def handler(request):
    """Handle OAuth initiation for Azure"""
    try:
        ...
        auth_url = oauth_manager.generate_auth_url('azure', encoded_state)
        ...
        await supabase_manager.log_activity(
            ...
            status='success',
            message='Azure OAuth flow initiated',
        ...
        logger.error(f"Error initiating Azure OAuth: {str(e)}")
        ...
        if user_id:
            await supabase_manager.log_activity(
                ...
                metadata={'provider': 'azure', 'error': str(e)}
            )
