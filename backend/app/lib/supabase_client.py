...
class SupabaseManager:
    ...
    async def save_integration(
        self, user_id: str, provider: str, token_data: Dict, user_info: Dict
    ) -> bool:
        ...
        try:
            ...
            result = (
                self.client.table("integrations").upsert(integration_data).execute()
            )

            if result.data:
                logger.info(
                    f"Successfully saved {provider} integration for user {user_id}"
                )
                return True
            else:
                logger.error(
                    f"Failed to save {provider} integration for user {user_id}"
                )
                return False
    ...
    async def deactivate_integration(self, user_id: str, provider: str) -> bool:
        ...
        try:
            ...
            if result.data:
                logger.info(
                    f"Successfully deactivated {provider} integration for user {user_id}"
                )
                return True
            else:
                logger.error(
                    f"Failed to deactivate {provider} integration for user {user_id}"
                )
                return False
    ...
    async def log_activity(
        ...
        try:
            activity_data = {
                ...
                "status": status,
                "description": message,  # Changed from 'message' to 'description'
            ...
            if result.data:
                logger.info(
                    f"Successfully logged activity: {activity_type} for user {user_id}"
                )
                return True
            else:
                logger.error(
    ...
    async def log_opportunity(
        ...
        """Log opportunity analysis to the opportunity_logs table"""
        try:
            import hashlib

            # Create email hash for deduplication (GDPR compliant)
            email_content = f"{email_data.get('from', '')}{email_data.get('to', '')}{email_data.get('subject', '')}{email_data.get('content', '')}"
            ...
            opportunity_data = {
                ...
                "opportunity_detected": ai_result.get("is_sales_opportunity", False),
                "confidence_score": ai_result.get("confidence", 0),
                "reasoning": ai_result.get("reasoning", ""),
                "metadata": {
                    "ai_result": ai_result,
                    "pipedrive_result": pipedrive_result,
                    "email_thread_count": len(email_data.get("email_thread", [])),
            ...
            result = (
                self.client.table("opportunity_logs").insert(opportunity_data).execute()
            )

            if result.data:
                logger.info(
                    f"Successfully logged opportunity analysis for user {user_id}"
                )
                return True
            else:
                logger.error(f"Failed to log opportunity analysis for user {user_id}")
                return False

        except Exception as e:
            logger.error(f"Error logging opportunity: {str(e)}")
            return False
